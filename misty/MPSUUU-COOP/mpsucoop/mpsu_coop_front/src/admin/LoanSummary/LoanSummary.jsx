...
const LoanSummary = () => {
  ...
  useEffect(() => {
    const fetchCurrentYearData = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/yearly-summary/current/');
  ...
  useEffect(() => {
    const fetchHistoricalYears = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/yearly-summary/all/');
    ...
    const fetchPenaltyData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/penalties/yearly/${selectedYear}/`,
          {
            headers: {
...
// recently lang ends
  ...
  try {
    const response = await axios.get(`http://localhost:8000/api/yearly-summary/${year}/`);
    setYearlyData(response.data);
  } catch (err) {
...
// chicha ends
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/api/total-penalties/')
      .then(res => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/api/total-fees-breakdown/')
      .then(res => {
