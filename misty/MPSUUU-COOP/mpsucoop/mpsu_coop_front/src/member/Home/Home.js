    ...
    Object.entries(groupedByOR).forEach(([orNumber, items]) => {
      ...
      rows.push(
        <tr key={`adv-${loanType}-${orNumber}`}>
          ...
          <td className="font-weight-700" style={{ color: loanType === 'Regular' ? '#1e7e34' : '#dc3545' }}>
            ₱{formatNumber(sum)}
          </td>
          <td>
            {first.date_paid ? formatISODate(String(first.date_paid).slice(0,10)) : '—'}
  ...
  const renderTableWithORGrouping = (filteredSchedules, loanType) => {
    ...
    Object.entries(groupedByOR).forEach(([orNumber, orSchedules]) => {
        ...
        <tr key={`${firstSchedule.id || firstSchedule.control_number}-main`}>
          ...
          <td>
            {(() => {
              ...
              return <span className="date-not-recorded">Paid (date not recorded)</span>;
            })()}
          </td>
          <td className="font-weight-600">
            <div className="or-group-container">
              {hasMultiple && (
                <span 
                  onClick={(e) => {
                    e.stopPropagation();
      ...
      if (hasMultiple && isOpen) {
        orSchedules.slice(1).forEach((schedule, idx) => {
          rows.push(
            <tr key={`${schedule.id || schedule.control_number}-${idx}`} className="expanded">
              <td>
                <span className={`loan-type-badge ${loanType.toLowerCase()}`}>
                  {schedule.loan_type || 'N/A'}
                </span>
              ...
              <td className="font-weight-600 or-column">
                {schedule.or_number}
              </td>
            </tr>
          );
  ...
  const renderHistoryWithORGrouping = (historyItems, loanType) => {
    ...
    Object.entries(groupedByOR).forEach(([orNumber, items]) => {
        ...
        <tr key={`hist-${loanType}-${orNumber}-main`}>
          ...
          <td>
            {(() => {
              const datePaid = first.date_paid || first.payment_date;
              if (datePaid) return formatISODate(String(datePaid).slice(0,10));
              return <span className="date-not-recorded">—</span>;
            })()}
          </td>
          <td className="font-weight-600">
            <div className="or-group-container">
              ...
              <span>{first.or_number}</span>
              {hasMultiple && (
                <span className="or-count">({items.length})</span>
              )}
            </div>
          </td>
          <td>
            <span className={`status-badge ${first.status === 'Advance' ? 'advance' : 'settled'}`}>
              {first.status === 'Advance' ? '✓ Advance' : '✓ Settled'}
            </span>
          </td>
        </tr>
      ...
      if (hasMultiple && isOpen) {
        items.slice(1).forEach((it, idx) => {
          rows.push(
            <tr key={`hist-${loanType}-${orNumber}-${idx}`} className="expanded">
              <td>
              ...
              <td>
                <span className={`status-badge ${it.status === 'Advance' ? 'advance' : 'settled'}`}>
                  {it.status === 'Advance' ? '✓ Advance' : '✓ Settled'}
                </span>
              </td>
            </tr>
    ...
    <div className="home-container">
      ...
      <div className="home-content">
        <div className="home-wrapper">
          ...
          <div className="main-grid">
            ...
            <div className="left-panel">
              ...
              <div className="member-profile-section">
                ...
                <div className="member-info-card">
                  ...
                  <div className="capital-share-container">
                    <div className="capital-share-item">
                      💰Share Capital: ₱{formatNumber(Math.min(memberData.share_capital || 0, SHARE_CAPITAL_LIMIT))}
                    </div>
              ...
              {/* Loan Fees Cards */}
              <div className="loan-fees-container">
                <div className="loan-fees-card">
                  <div className="loan-fees-title">
                    🏦 Regular Loan Fees & Interest{nearestRegularLoan?.status !== 'Paid-off' && ` - Year ${currentRegularYear}`}
                  </div>
                  {nearestRegularLoan?.status === 'Paid-off' ? (
                    <>
                      <p className="loan-fees-item">
                        <strong>Interest (8%):</strong> ₱0.00
                      </p>
                      <p className="loan-fees-item">
                        <strong>CISP:</strong> ₱0.00
                ...
                <div className="loan-fees-card emergency">
                  <div className="loan-fees-title">
                    🚨Emergency Loan Fees & Interest{nearestEmergencyLoan?.status !== 'Paid-off' && ` - Month ${currentEmergencyYear}`}
                  </div>
                  {nearestEmergencyLoan?.status === 'Paid-off' ? (
                ...
                <div className="loan-buttons-container">
                  <button
                    onClick={() => handleNavigation('/accounts')}
                    className="loan-button share-capital"
                  >
                    📊 Share Capital Transactions
                  </button>
                  ...
                  <button
                    onClick={() => handleNavigation('/loans')}
                    className="loan-button bi-monthly"
                  >
                    💳 Bi Monthly Amortization
                  </button>
                </div>
            ...
            <div className="right-panel">
              ...
              <div className="loan-details-grid">
                ...
                {/* Regular Loan Details Card */}
                <div className="loan-details-card regular">
                  <h3 className="loan-details-title">
                    🏦 REGULAR LOAN DETAILS
                  </h3>
                  ...
                  {nearestRegularPaymentSchedule ? (
                    <div className="loan-details-content">
                      ...
                      <div className="loan-details-row">
                        <div className="loan-detail-item">
                          <p className="loan-detail-label">
                            <strong>Regular Loan:</strong>
                          </p>
                          <p className="loan-detail-value regular">
                            ₱{formatNumber(parseFloat(nearestRegularLoan?.loan_amount || 0).toFixed(2))}
                          </p>
                        </div>
                        
                        <div className="loan-detail-item">
                          <p className="loan-detail-label small">
                            <strong>Loan Approval Date:</strong>
                          </p>
                          <p className="loan-detail-value medium">
                            {nearestRegularLoan?.loan_date ? formatDate(nearestRegularLoan.loan_date) : 'N/A'}
                          </p>
                        </div>
                        ...
                        <div className="loan-detail-item">
                          ...
                          <p className={`loan-detail-value medium ${nearestRegularLoan?.status === 'Paid-off' ? 'settled' : ''}`}>
                            {nearestRegularLoan?.status === 'Paid-off' 
                              ? 'SETTLED' 
                              : nearestRegularPaymentSchedule?.due_date 
                                ? new Date(nearestRegularPaymentSchedule.due_date).toLocaleDateString()
                        ...
                        <div className="loan-detail-item">
                          ...
                          <p 
                            className="loan-detail-value medium control"
                            onClick={() => navigate(`/loans?control=${nearestRegularLoan?.control_number}`)}
                          >
                            {nearestRegularLoan?.control_number || 'N/A'}
                          </p>
                      ...
                      {/* Second Row */}
                      <div className="loan-details-row balance">
                        <div className="loan-detail-item balance">
                          <p className="loan-detail-label">
                            <strong>Paid Amount:</strong>
                          </p>
                          <p className="loan-detail-value large">
                            ₱{formatRemainingBalance(calculatePaidBalance().toFixed(2))}
                          </p>
                        </div>
                    ...
                    <div className="no-loan-message regular">
                      <p className="no-loan-text">
                        No upcoming Regular Loan Due.
                      </p>
                    </div>
                  )}
                ...
                {/* Emergency Loan Details Card */}
                <div className="loan-details-card emergency">
                  <h3 className="loan-details-title">
                    🚨 EMERGENCY LOAN DETAILS
                  </h3>
                    ...
                    <div className="loan-details-content">
                      ...
                      <div className="loan-details-row">
                        <div className="loan-detail-item">
                          ...
                          <p className="loan-detail-value emergency">
                            ₱{formatNumber(parseFloat(nearestEmergencyLoan?.loan_amount || 0).toFixed(2))}
                          </p>
                        </div>
                        ...
                        <div className="loan-detail-item">
                          ...
                          <p className={`loan-detail-value medium ${nearestEmergencyLoan?.status === 'Paid-off' ? 'settled' : ''}`}>
                            {nearestEmergencyLoan?.status === 'Paid-off' 
                              ? 'SETTLED' 
                              : nearestEmergencyPaymentSchedule?.due_date 
                                ? new Date(nearestEmergencyPaymentSchedule.due_date).toLocaleDateString()
          ...
          {/* PAYMENT TABLES SECTION */}
          <div className="payment-tables-section">
            
            {/* Payment Tables */}
            <div className="payment-tables-grid">
              
              {/* Regular Loan Payment Table Section */}
              <div className="payment-table-container regular">
                {/* Regular History Button */}
                <div className="table-buttons-container regular">
                  <button
                    onClick={() => { setShowRegularHistory(true); setShowRegularAdvanceOnly(false); }}
                    className="table-button history"
                  >
                    CLICK TO VIEW PAYMENTS HISTORY
                  </button>
                  <button
                    onClick={() => setShowRegularAdvanceOnly(true)}
                    className={`table-button advance ${showRegularAdvanceOnly ? 'active' : ''}`}
                  >
                    ADVANCE PAYMENT
                  </button>
                </div>
                ...
                {/* Regular Loan Payment Table */}
                <div className="payment-table-wrapper regular">
                  <div className="payment-table-header regular">
                    <h3 className="payment-table-title">
                      🟢 Regular Loan Payments
                    </h3>
                  </div>
                  <div className="payment-table-scroll hide-scrollbar">
                    {showRegularAdvanceOnly ? (
              ...
              {/* Emergency Loan Payment Table Section */}
              <div className="payment-table-container emergency">
                {/* Emergency History Button */}
                <div className="table-buttons-container emergency">
                  <button
                    onClick={() => { setShowEmergencyHistory(true); setShowEmergencyAdvanceOnly(false); }}
                    className="table-button history emergency"
                  >
                    CLICK TO VIEW PAYMENTS HISTORY
                  </button>
                  <button
                    onClick={() => setShowEmergencyAdvanceOnly(true)}
                    className={`table-button advance emergency ${showEmergencyAdvanceOnly ? 'active' : ''}`}
                  >
                    ADVANCE PAYMENT
                  </button>
                </div>
                ...
                {/* Emergency Loan Payment Table */}
                <div className="payment-table-wrapper emergency">
                  <div className="payment-table-header emergency">
                    <h3 className="payment-table-title">
                      🔴 Emergency Loan Payments
                    </h3>
                  </div>
